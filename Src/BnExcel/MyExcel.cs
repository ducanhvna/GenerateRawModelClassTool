...
namespace BnExcel
{
    public class MyExcel
    {
        ...
        public static Excel.Worksheet Sheets(string sheetName)
        {
            return (Excel.Worksheet)MyBook.Sheets[sheetName];
        }
        ...
        /// <returns></returns>
        public static List<List<string>> ReadDataByColum(string sheetName,int startRow, int startColumn, List<int> ListColumnToRead, int limitRow =10000)
        {
            var xlsSheet = (Excel.Worksheet)MyBook.Sheets[sheetName];
            List<List<string>> result = new List<List<string>>();
            int row = startRow;
            while(row<limitRow&& xlsSheet.Cells[row, startColumn].Value !=null&&(string)xlsSheet.Cells[row, startColumn].Value != "")
            {
